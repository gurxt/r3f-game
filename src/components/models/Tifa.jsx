...
export default function Tifa(props) {
  ...
  const handleClick = e => {
    e.stopPropagation()
    const _keys = Object.keys(actions).filter(_action => !_action.includes("Tifa"))
    setAnimationIdx(prev => (prev + 1) % _keys.length)
  }

  useEffect(() => {
    const _keys = Object.keys(actions)
    actions[_keys[animationIdx]].reset().fadeIn(0.5).play()

    return () => { actions[_keys[animationIdx]].fadeOut(0.5) }
  }, [animationIdx])
